...
FString FNsBonsaiNameBuilder::SanitizeNamePartPreserveCase(const FString& InValue)
{
...
TArray<FString> FNsBonsaiNameBuilder::SanitizeAssetNameParts(const FString& InAssetName, const UNsBonsaiSettings& Settings)
{
    ...
    for (const FString& Part : Parts)
    {
        FString Clean = SanitizeNamePartPreserveCase(Part);
        if (Clean.IsEmpty())
        {
            continue;
        }
...
FString FNsBonsaiNameBuilder::BuildFinalAssetName(const FNsBonsaiNameBuildInput& Input, const UNsBonsaiSettings& Settings)
{
    ...
    return Base.IsEmpty() ? SanitizeVariantToken(Input.VariantToken) : (Base + Separator + SanitizeVariantToken(Input.VariantToken));
}
